...
def get_tables(sql_stmt):
    parsed = sp.parse(sql_stmt)[0]
    from_token = next(token for i, token in enumerate(parsed.tokens) if isinstance(token, sp.sql.IdentifierList)
                      and str(parsed.token_prev(i, skip_cm=True)[1]) == 'FROM')
...
def get_where(sql_stmt):
    parsed = sp.parse(sql_stmt)[0]
    where_token = next((token for token in parsed.tokens if isinstance(token, sp.sql.Where)), None)
    try:
        # print(select_stmt.parseString(where_token.value).dump())
        print(sp.format(sql_stmt, reindent=True))
    except ParseException as pe:
        print(pe.msg)
    if where_token is not None:
        return where_token.value.lower().replace(';', '').split()[1:]


if __name__ == "__main__":
    tests = """select * from xyzzy where z > 100
        select * from xyzzy where z > 100 order by zz
        select * from xyzzy
        select * from xyz where x like '%(voice)%' AND y = 1""".splitlines()
    for t in tests:
        print(t)
        # try:
